...
module.exports.handleEvent = async function({ api, event }) {
  ...
  if (body && body.trim().toLowerCase() === "nobita") {
  ...
  // Prompt
  const prompt = `
Tum ek naughty, romantic, confident ladka ho — jiska naam nobita hai.
...
`;
  ...
  try {
    ...
    return api.sendMessage("Sorry baby 😅 nobita abhi thoda busy hai...", threadID, messageID);
  }
};

module.exports.run = async function({ api, event }) {
  return api.sendMessage("Mujhse baat karne ke liye pehle 'nobita' likho, phir mere message ka reply karo 😎", event.threadID, event.messageID);
};
